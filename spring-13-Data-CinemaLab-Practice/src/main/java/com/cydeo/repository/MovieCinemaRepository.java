...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    Optional<MovieCinema> findById(Long id);//optional id

    //Write a derived query to count all movie cinema with a specific cinema id
    Integer countAllByCinemaId(Long id);

    //Write a derived query to count  all movie cinema with a specific movie id
    Integer countAllByMovieId(Long id);

    //Write a derived query to list all movies cinemas with higher than specific date
    List<MovieCinema> findAllByDataTimeAfter(LocalDateTime dateTime);

    //Write a derived query to find top3 expensive movies
    ...
    //---------------------------JPQL QUERIES--------------------------------//
    //Write a JPQL query to list all movie cinemas with higher than specific date

    @Query("SELECT  mc FROM MovieCinema  mc WHERE mc.dataTime>?1")//something
